...
(function($){
  jQuery.fn.buildContainers = function (options){
    return this.each (function (){
      ...
      if(!isStructured){
        ...
        container.empty();
        var structure=''+
                '<div id="dock" style="margin-left:150px; margin-bottom: -30px; position:abosolute; z-index:50;"> </div><div class="no"><div class="ne"><div class="n"></div></div>'+
  ...
  jQuery.fn.containerSetIcon = function (icon,path){
    var container=$(this);
    if (icon && icon!="" ){
      container.find(".ne:first").prepend("<img class='icon' src='"+path+"icons/"+icon+"' style='position:absolute; margin-top:-20px; margin-left:-40px;'/>");
      container.find(".n:first").css({paddingLeft:55});
    }else{
  ...
  jQuery.fn.containerSetButtons = function (buttons,opt){
    ...
    if (buttons !=""){
      ...
      for (var i in btn){
        ...
        if (btn[i]=="i"){
          container.find(".buttonBar:first").append("<img src='"+path+container.attr('skin')+"/close.png' class='iconizeContainer'/>");
          container.find(".iconizeContainer:first").bind("click",function(){container.containerIconize(opt);});
        }
        if (btn[i]=="f"){
          container.find(".buttonBar:first").append("<img src='"+path+container.attr('skin')+"/iconize.png' class='fullContainer'/>");
          container.find(".fullContainer:first").bind("click",function(){container.mb_fullscreen();});
        }   
      }
  ...
  jQuery.fn.mb_iconizeclose = function (){
    return this.each(function(){
      var container=$(this);
      var opt= container.get(0).options;
      var el=container.get(0);
