 ...
 */
 ...
 function tag_sort () {
	...
	var tag = null,tag_post=null;

	$('.blog_post').fadeIn();
	$('.point a').click (function () {
//	$('.point a').triggerHandler("click");
		tag=$(this).attr('title');
		$('.blog_post').each(function () {
			tag_post=$(this).attr('tag');
			if ( tag_post != tag){
				$(this).fadeOut();
			}
		});
...
(function($){
     ...
     $.fn.tagcloud.defaults = {
         ...
         max_zoom: 200,
         min_zoom: 200,
         zoom_factor: 1, //multiplication factor for wheel delta
         rotate_factor: -0.45, // multiplication factor for rotation
         fps: 10, // frames per second
         centrex: 166, // set centre of display
         centrey: 166,
	 ...
	 var points = [];
         ...
         if (options.background_colour){
             bg_colour = parsecolour('#000');
         }else{
             bg_colour = parsecolour('#000');
         }
         if (options.foreground_colour){
             fg_colour = parsecolour('#fff');
         }else{
             fg_colour = parsecolour('#fff');
         }
         ...
         function draw(){
             ...
             if(dirty){
	             ...
	             var u = (depth - eyez)/(points.data[s_p].z - eyez);
                     ...
                     var dist = Math.round(((maxz - points.data[s_p].z)/diffz) * 100 + 370);
                     var dist_colour = getcolour(dist-300);                 
                     //set new 2d positions for the data
                     $('#'+points.data[s_p].id + ' a', container).css({'color':'rgb(255,255,255)','background-color':'rgb(0,0,0)', 'border':'6px solid', 'border-color':'rgb(0,0,0)', 'border-radius':6, '-moz-border-radius':6,'-iwebkit-border-radius':3});
         ...
         $('li a', container).each(function(idx, val){
                                       ...
                                       container.append('<div id="'+ id_stub + idx +'" class="point" style="position:absolute;display:none;"><a href="' + $(this).attr('href')  + '" title="' + $(this).attr('title') + '" onmouseover="tag_sort()">' + $(this).html()  + '</a></div>');
